...
class QueryBuilderTest extends TestCase
{
    ...
    {
        $this->assertEquals('WHERE amount = 3 ', $this->queryBuilder->where('amount', '=', '3')->getQuery());
    }

    public function testWhereIn(): void
    {
        $this->assertEquals(
            "WHERE testvalue IN ('testvalue','anothervalue') ",
    ...
    {
        $this->assertEquals(
            'LEFT JOIN tableName ON firstColumn = secondColumn ',
    ...
    {
        $this->assertEquals('AND test = something ', $this->queryBuilder->andOr(
            'AND',
            'test',
            '=',
            'something'
    ...
    {
        $this->assertEquals(
            "UPDATE tableName SET name = Boris WHERE age = 25 ",
            $this->queryBuilder->updateRecords(
                'tableName',
                'name',
                '=',
                'Boris',
    ...
    {
        $this->assertEquals(
            'DELETE FROM tableName WHERE age = 25 ',
